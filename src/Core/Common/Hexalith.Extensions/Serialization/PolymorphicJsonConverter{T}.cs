    ...
    where T : class, IPolymorphicSerializable
{
    ...
    public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        _ = jsonDoc.Deserialize(PolimorphicSerializableTypeMapper.GetType(typeName, majorVersion, minorVersion));

        // Deserialize the remaining properties
        return JsonSerializer
            .Deserialize(
                jsonDoc.RootElement.GetRawText(),
                PolimorphicSerializableTypeMapper.GetType(typeName, majorVersion, minorVersion),
                options) as T
